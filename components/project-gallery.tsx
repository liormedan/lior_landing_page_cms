              ...
              >
                ...
                <div className="flex flex-1 flex-col p-6 text-right">
                  ...
                  {project.ctaHref && (
                    <div className="mt-4">
                      <Link
                        ...
                        className="inline-flex flex-row-reverse items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-700 focus-visible:ring-offset-2"
                      >
                        {project.ctaLabel ?? '׳׳§׳¨׳™׳׳× ׳¢׳•׳“'}
                        <svg
                          className="h-4 w-4"
                  ...
                  <button
                    ...
                    className="mt-6 inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:underline hover:text-slate-900 underline focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2"
                    aria-expanded={isExpanded}
                    aria-controls={`project-${index}-details`}
                    aria-label={`${isExpanded ? '׳¡׳’׳™׳¨׳×' : '׳₪׳×׳™׳—׳×'} ׳₪׳¨׳˜׳™ ׳”׳₪׳¨׳•׳™׳§׳˜ ${project.title}`}
                    type="button"
                  >
                    {isExpanded ? '׳¡׳’׳™׳¨׳× ׳₪׳¨׳˜׳™ ׳”׳₪׳¨׳•׳™׳§׳˜' : '׳׳₪׳¨׳˜׳™׳ ׳ ׳•׳¡׳₪׳™׳'}
                    <svg
                ...
                {isExpanded && (
                  ...
                  >
                    <div>
                      <h4 className="text-sm font-semibold text-slate-900">׳׳” ׳׳§׳‘׳׳™׳:</h4>
                      <ul className="mt-3 space-y-2 text-right" role="list">
                        {project.features.map((feature, featureIndex) => (
                          <li key={featureIndex} className="flex items-start justify-start gap-2" role="listitem">
                            <span className="mt-1 inline-block h-2 w-2 flex-shrink-0 rounded-full bg-slate-900" aria-hidden="true" />
                            <span>{feature}</span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {project.examples.map((example, exampleIndex) => (
                          <span
                            key={exampleIndex}
                            className="inline-flex items-center rounded-full bg-slate-200/80 px-3 py-1 text-xs font-medium text-slate-700 shadow-sm"
                          >
                            {example}
                          </span>
                        ))}
